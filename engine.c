...
struct Tuple window_wrt_map;

void reallocate_map(){
    if (window_wrt_map.y + LINES > map_size.y){
        char **new_map = malloc(sizeof(char *)*(map_size.y+LINES));
        for (int i=map_size.y-1; i < map_size.y+LINES; i++){
    ...
    if (window_wrt_map.y == 0){
        ...
        fill_map(start, end);
        map_size.y += LINES;
        window_wrt_map.y += LINES;
        for (int i=0; i < num_elems; i++){
            elements[i].start.y += LINES;
        }
    }
    if (window_wrt_map.x == 0){
        ...
        fill_map(start, end);
        map_size.x += COLS;
        window_wrt_map.x += COLS;
        for (int i=0; i < num_elems; i++){
            elements[i].start.x += COLS;
        }
    }
    if (window_wrt_map.x + COLS > map_size.x){
        for (int y = 0; y < map_size.y; y++){
            char *new_row = malloc(sizeof(char)*(map_size.x+COLS));
...
void print_map(){
    for (int y = 0; y < LINES; y++){
        for (int x = 0; x < COLS; x++){
            int map_y = window_wrt_map.y + y;
            int map_x = window_wrt_map.x + x;
